...
::: callout-important
This lab is due on Friday, Sept 4 at 11:59pm. To be considered on time, the following must be done by the due date:
...
b. Now, copy and paste the code below into your code chunk within your quarto template. Edit the provided code so that the dataset is filtered to "Residential" development areas, the same as part (a). Then, run the code to create a table displaying the number of bike crashes in **each age group** for residential areas. 
...
Run the following code to create grouped box plots. (The code should already be in your Quarto template.) Add an informative title and informative axis labels. What do you notice about `Snow, Sleet, Hail, Freezing Rain/Drizzle` driver ages in Rural vs. Urban areas?
