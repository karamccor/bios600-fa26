---
title: "Lab 4: Practice with ggplot2 and tidyverse"
format: html
---

::: callout-important
This lab is due on Monday, September 14 at 11:59pm. To be considered on time, the following must be done by the due date:
...
1. In Canvas, you'll find the template for this assignment ("Lab 04 Template"). Download the file and open it. 

2. Save it in your class project folder as "`lab-04.qmd`".

3. Open the document. Replace "Your Name Here" with your name in the YAML (the header of the document). Also, replace "Today's Date Here" with today's date. Filling in your name and date will count towards your "Formatting" score from here on out.

4. Render your document to make sure your YAML is rendering correctly.
...
In the code chunk labeled `load-packages`, copy and paste the code below to load in the tidyverse package for today's lab:
...
## Globally setting warnings and messages to false

Note that in your template, warnings and message have been **set to false globally** (i.e. across the entire document) by specifying these options in the YAML. Since we will be loading in packages and a dataset, this will be helpful for you formatting on this assignment.

It can be helpful to set global options this way in practice. However, take care when choosing this route, as sometimes warnings and messages can be helpful for troubleshooting. 

:::

## Practice with ggplot and tidyverse

Today’s lab involves more practice with ggplot and the tidyverse. You will also learn about loading data into R and setting a working directory to look for data.
...
At the beginning of class, you should have created a `data` folder that lives in your project folder. If you copy your data (perhaps it's called `file.csv`) into your `data` folder, you can read it in with the following code: 
...
a. In your code chunk labeled `load-data`, read in the data with the following code:

```{r}
#| echo: true
#| eval: false

mcd <- read.csv("data/menu.csv")
```

Watch out for the naming of the file and where it’s saved on your computer! Print the first seven rows of the dataset with `slice()`. 

::: callout-important

## Getting an error?

If you're getting an error here, check the following:

- That you have your project open within RStudio (there is a blue cube at the top right of your RStudio window with the name of your project folder next to it. If you don't see this, click File > Open Project... and navigate to your project folder.)

- That there is a folder called `data` within this project folder, and `menu.csv` is saved in that `data` folder.

If you've checked these things and are still getting an error, reach out to the teaching team for help!
