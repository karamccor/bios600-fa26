...
<!--# Lecture too long, end after introducing the Poisson dist-->
...
- Lab 4 session today at 3:30pm. Lab 4 is due **Monday September 14 at 11:59pm**. (Adding some time due to shorter week.)

- HW 2 due **Thursday at 11:59pm**.

## Announcements

- Today, we will do our first Application Exercise (AE). You will get a chance to practice more applied coding.

- AEs will be turned in on Canvas only (not Gradescope).

- Today's AE is due **Wednesday September 9 at 11:59pm**. This will count as a participation grade.
...
-   **Discrete** random variables are those that can take on a countable number of potential values (could be countably infinite!), each associated with a probability of occurring.
    ...
    - Examples: Tossing a coin (H, T), Rolling a die (1, 2, 3, 4, 5, 6)
...
Let's verify that the three rules above are satisfied with the births example. 
...
With the notation that event $X=1$ is a "success" and $X=0$ is a "failure", 
...
- Note: $P(X = 1)$ reads: "The probability that the random variable $X$ takes on the value $1$."
...
## Note on Bernoulli random variables
...
Let's use this logic to fill out our table. 

## Extending the Bernoulli distribution



| 1st Birth $X_1$ | 2nd Birth $X_2$ | \# Preterm Births $Z$ | Probability of Outcome                      |
|---------------|---------------|---------------|-----------------------|
| 0             | 0             | 0                     |      |
| 1             | 0             | 1                     |                                             |
| 0             | 1             | 1                     |                                             |
...
The **binomial distribution** gives the probability of $k$ "successes" from a sequence of $n$ independent Bernoulli trials. 
...
Let's solve this using the binomial distribution.

First, we need to ensure the assumptions are met: 
...
So, we can define $Z$ as the number of preterm births in 3 randomly sampled births.

Then, $Z \sim Binom(3, 0.1)$.
...
Suppose you have 4 independent trials, and you are interested in the probability that **at least one trial is a success**. 

You could write this out as:

$P(X \geq 1) = P(X=1) + P(X=2) + P(X = 3) + P(X = 4)$

Alternatively, you could write it out as:

$P(X \geq 1) = 1 - P(X = 0)$

**This rule will come in handy in your Application Exercise.**
...
- When you've completed the exercises, render, save to PDF, and submit on Canvas> Assignments. Due **Wednesday September 9 at 11:59pm**.
...
To model this question using a Poisson random variable, let's verify that the assumptions are met:
...
Now that we've verified that our random variable can be modeled using a Poisson distribution, let's define it:

Let $Z$ represent the deaths due to Alzheimer's disease in a town each year. 

Then, $Z$ can be modeled by a Poisson distribution with rate 1.5. 

In statistical terms: $Z \sim \textrm{Poisson}(\lambda = 1.5)$ 

## The Poisson distribution

$Z \sim Pois(1.5)$, and $P(Z=0)$, $P(Z=1)$, and $P(Z\geq 2)$ are disjoint events. So, $P(Z \geq 2) = 1- [P(Z = 0) + P(Z = 1)]$, where
