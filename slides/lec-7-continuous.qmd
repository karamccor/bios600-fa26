---
title: "Lecture 7: Poisson & Continuous distributions"
...
- HW 3 (short) will be released after class, due next **Wednesday 9/16** at 11:59pm.

- AE 01 solution will be posted after class

## Exam Info

- Exam 01 will be in-class on Tuesday, September 22. 

- Today's lecture marks the end of the content for Exam 01. (Today's lecture is included in Exam 01 content.) 

- Topics list and Formula sheet have been added to Exam 01 Preparation on Canvas.
  - The formula sheet will be provided to you on the exam.
  
- Exam review questions will also be posted by end of the day on Friday.

## Announcements

- Lab session on 9/15 will be **general TA Office Hours**. (No new lab assignment that week.)

- **No lab session 9/22** (day of exam). 
...
- Last time, we verified that our example follows the assumptions of a Poisson distribution.  


## Example

- What is the probability of at least two deaths due to Alzheimer's disease in a year?

- Let's define $Z$, a randon variable, as the number of Alzheimer's death in 1 year. 

- Then $Z \sim Pois(1.5)$, and $P(Z=0)$, $P(Z=1)$, and $P(Z\geq 2)$ are disjoint events. 

## Example

- Using the Complement Rule, $P(Z \geq 2) = 1- [P(Z = 0) + P(Z = 1)]$. 

- So, we just need to find $P(Z = 0)$ and $P(Z = 1)$ using the Poisson distribution, and plug in to solve. 
...
Make sure each parentheses has a pair! Common source of Quarto files not rendering...
...
Head to Canvas for today's application exercise.

When you've completed the exercise, render, save to PDF, and submit on Canvas > Assignments. Due **Friday Sept. 11 at 11:59pm**.
...
Let $X$ be the outcome of rolling a fair die. Each face is equally likely:
...
On average, a die roll comes out to **3.5**. 

Note that this isn't even a possible outcome, which illustrates that $E(X)$ describes the long-run average of the *distribution*, not any single roll.
...
- Think: similar to Bernoulli. 

- What is the relationship between a Bernoulli and a Binomial random variable?
...
- That is, the expectation is simply the event rate. The variance is also the event rate.
...
::: callout-note
This formulas will be provided to you if needed.
:::
...
-   So, a z-score of 1.2 is 1.2 standard deviations above a mean.

- A z-score of -0.8 is 0.8 standard deviations below the mean.
...
# End of Exam 01 Material

## Recap

- Poisson distribution and properties
